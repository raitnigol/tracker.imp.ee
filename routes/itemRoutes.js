...
function deleteItemById(id, callback) {
    const sql = 'DELETE FROM main_items WHERE id = ?';
...
router.post('/main_items', verifyToken, (req, res) => {
    const { item, bought_for, sold_for } = req.body;
    const user_id = req.userId;  // Retrieved from token verification middleware

    const sql = `INSERT INTO main_items (item, bought_for, sold_for, user_id) VALUES (?, ?, ?, ?)`;
...
router.get('/main_items', verifyToken, (req, res) => {
    const userId = req.userId;
    db.all(`SELECT * FROM main_items WHERE user_id = ?`, [userId], (err, rows) => {
        if (err) {
...
router.get('/main_items/:id', verifyToken, (req, res) => {
    const { id } = req.params;
    db.get(`SELECT * FROM main_items WHERE id = ?`, [id], (err, row) => {
        if (err) {
            console.error("Database error:", err);
...
router.put('/main_items/:id', verifyToken, (req, res) => {
    const { id } = req.params;
    const { item, bought_for, sold_for } = req.body;
    const sql = `UPDATE main_items SET item = ?, bought_for = ?, sold_for = ? WHERE id = ?`;
...
router.delete('/main_items/:id', verifyToken, (req, res) => {
    const { id } = req.params;
...
// GET endpoint to fetch sub-items for a specific main item
router.get('/main_items/:id/sub-items', verifyToken, (req, res) => {
